...
def get_pawns_moves(bitboards, color, en_passant_target=None):
	...
	if color == WHITE:
		...
		while pawns:
			...
			while captures_board:
				lsb = captures_board & -captures_board
...
def get_knights_moves(bitboards, color):
	...
	while knights:
		...
		while quiets_board:
			lsb = quiets_board & -quiets_board
			target = lsb.bit_length() - 1
			quiets_board &= quiets_board - 1
